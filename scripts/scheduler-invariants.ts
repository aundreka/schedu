...
function recurringRow(id: string, lessonPlanId: string, day: string, start: string, end: string, meetingType: "lecture" | "laboratory") {
  return entry({
    plan_entry_id: id,
...
function testLockedMultiSlotPlacement() {
  ...
  const result = generateSchedulePlan(input);
  const locked = result.entries.find(
    (e) =>
      e.is_locked &&
      e.title === "Lesson 1" &&
  ...
  assert.ok(locked, "Expected locked lesson entry to be preserved.");
  assert.equal(locked?.scheduled_date, "2026-06-01");
  assert.equal(locked?.start_time, "10:00:00");
  assert.equal(locked?.end_time, "11:00:00");
}
...
function testFinalExamDoesNotOverwriteLastOccupiedMeeting() {
  ...
  const result = generateSchedulePlan(input);
  const lockedLast = result.entries.find(
    (e) =>
      e.is_locked &&
  ...
  assert.ok(lockedLast, "Expected locked non-lesson entry on last slot.");
  assert.equal(lockedLast?.scheduled_date, "2026-06-05");
  assert.equal(lockedLast?.start_time, "08:00:00");

  const finalExam = result.entries.find((e) => e.category === "exam" && e.title.toLowerCase().includes("final"));
  assert.ok(finalExam, "Expected final exam to be present.");
  assert.notEqual(finalExam?.scheduled_date, "2026-06-05", "Final exam should not overwrite last occupied meeting.");

  const shiftedDiagnostic = result.diagnostics.constraints.find((d) => d.code === "FINAL_EXAM_SHIFTED");
  assert.ok(shiftedDiagnostic, "Expected FINAL_EXAM_SHIFTED diagnostic when last meeting is occupied.");
}
...
function testBlackoutDatesAreRespected() {
  ...
  assert.equal(scheduledOnBlackout.length, 0, "Expected no entries scheduled on blackout date.");
}
...
function testGeneratedEntryIdsAreUnique() {
  ...
  assert.equal(unique.size, ids.length, "Expected generated plan_entry_id values to be unique.");
}
