...
function weekdayPool(meetingsPerWeek) {
    if (meetingsPerWeek === 2)
        return ["monday", "wednesday"];
    if (meetingsPerWeek === 3)
        return ["monday", "wednesday", "friday"];
    if (meetingsPerWeek === 4)
        return ["monday", "tuesday", "thursday", "friday"];
    return ["monday", "tuesday", "wednesday", "thursday", "friday"];
}
function toWeekdayIndex(name) {
    if (name === "sunday")
        return 0;
    if (name === "monday")
        return 1;
...
function buildScenarioInput(scenario) {
    ...
    const recurringRows = weekdays.map((day, i) => ({
    ...
    const lessonRows = Array.from({ length: scenario.lessons }, (_, i) => ({
...
function evaluate(input, algorithmId, algorithmLabel, repeatIdx, seed) {
    const expectedCount = input.entries.filter((e) => e.category === "lesson").length +
        input.entries
    ...
    const output = withSeededRandom(seed, () => (0, lessonPlanScheduler_1.generateScheduledEntries)({ ...input, algorithm: algorithmId }));
    const elapsed = perf_hooks_1.performance.now() - started;
    const generated = output.filter((e) => e.scheduled_date && e.entry_type !== "recurring_class" && TARGET_CATEGORIES.has(e.category));
    ...
    const assessRows = generated.filter((e) => e.category === "written_work" || e.category === "performance_task" || e.category === "exam");
    let withinWindow = 0;
    for (const row of assessRows) {
        const t = parseDate(String(row.scheduled_date)).getTime();
        const pos = (t - start) / span;
        const [from, to] = WINDOW_BY_CATEGORY[row.category];
        if (pos >= from && pos <= to)
            withinWindow += 1;
    }
...
function pairedTTestPValue(diffs) {
    if (diffs.length < 2)
        return 1;
    const m = mean(diffs);
...
function wilcoxonSignedRankPValue(diffsRaw) {
    ...
    while (i < n) {
        let j = i;
        while (j + 1 < n && pairs[j + 1].abs === pairs[i].abs)
            j += 1;
...
function significance(rows) {
    ...
    const byAlgoAndPair = new Map();
    for (const algo of algorithms)
        byAlgoAndPair.set(algo, new Map());
    rows.forEach((r) => {
    ...
    for (let i = 0; i < algorithms.length; i += 1) {
        for (let j = i + 1; j < algorithms.length; j += 1) {
            ...
            const mapA = byAlgoAndPair.get(a) ?? new Map();
            const mapB = byAlgoAndPair.get(b) ?? new Map();
            const diffs = [];
...
function toLatexSummary(summary) {
    const lines = [];
    ...
    lines.push("\\hline");
    summary.forEach((r) => {
        lines.push(`${latexEscape(r.algorithm)} & ${r.objective_mean} $\\pm$ ${r.objective_ci95} & ${r.coverage_mean} & ${r.load_balance_mean} & ${r.runtime_mean_ms} $\\pm$ ${r.runtime_ci95_ms}\\\\`);
    });
    lines.push("\\hline");
    lines.push("\\end{tabular}");
    return `${lines.join("\n")}\n`;
}
function toLatexSignificance(rows) {
    const lines = [];
    ...
    lines.push("\\hline");
    rows.forEach((r) => {
        lines.push(`${latexEscape(r.algorithm_a)} & ${latexEscape(r.algorithm_b)} & ${r.mean_diff_a_minus_b} & ${r.paired_t_pvalue} & ${r.wilcoxon_pvalue}\\\\`);
    });
    lines.push("\\hline");
    lines.push("\\end{tabular}");
    return `${lines.join("\n")}\n`;
}
function toMarkdown(rows, summary, sig, repeats, seedBase) {
    const lines = [];
    ...
    summary.forEach((row, idx) => {
        lines.push(`| ${idx + 1} | ${row.algorithm} | ${row.n} | ${row.objective_mean} | ${row.objective_ci95} | ${row.coverage_mean} | ${row.lesson_order_mean} | ${row.assessment_window_mean} | ${row.load_balance_mean} | ${row.runtime_mean_ms} | ${row.runtime_ci95_ms} |`);
    });
    lines.push("");
    ...
    lines.push("|---|---|---:|---:|---:|---:|");
    sig.forEach((row) => {
        lines.push(`| ${row.algorithm_a} | ${row.algorithm_b} | ${row.n_pairs} | ${row.mean_diff_a_minus_b} | ${row.paired_t_pvalue} | ${row.wilcoxon_pvalue} |`);
    });
    lines.push("");
    ...
    lines.push("|---|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|");
    rows.forEach((row) => {
        lines.push(`| ${row.scenario} | ${row.repeat_idx} | ${row.seed} | ${row.algorithm} | ${row.objective_score} | ${row.coverage_pct} | ${row.lesson_order_pct} | ${row.assessment_window_pct} | ${row.load_balance_pct} | ${row.runtime_ms} | ${row.generated_items} |`);
    });
    lines.push("");
...
function run() {
    ...
    for (let si = 0; si < SCENARIOS.length; si += 1) {
        ...
        for (let ai = 0; ai < lessonPlanScheduler_1.SCHEDULER_ALGORITHMS.length; ai += 1) {
            const algo = lessonPlanScheduler_1.SCHEDULER_ALGORITHMS[ai];
    ...
    const base = (0, path_1.join)(process.cwd(), "generated", "scheduler-benchmarks", stamp);
    (0, fs_1.mkdirSync)(base, { recursive: true });
    (0, fs_1.writeFileSync)((0, path_1.join)(base, "benchmark_details.csv"), toCsv(rows), "utf8");
    (0, fs_1.writeFileSync)((0, path_1.join)(base, "benchmark_summary.csv"), toCsv(summary), "utf8");
    (0, fs_1.writeFileSync)((0, path_1.join)(base, "benchmark_significance.csv"), toCsv(sig), "utf8");
    (0, fs_1.writeFileSync)((0, path_1.join)(base, "benchmark_report.md"), toMarkdown(rows, summary, sig, repeats, seedBase), "utf8");
    (0, fs_1.writeFileSync)((0, path_1.join)(base, "benchmark_summary.tex"), toLatexSummary(summary), "utf8");
    (0, fs_1.writeFileSync)((0, path_1.join)(base, "benchmark_significance.tex"), toLatexSignificance(sig), "utf8");
