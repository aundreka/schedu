...
const buildPacingPlan_1 = require("./buildPacingPlan");
const PT_SUBCATEGORIES = [
    "activity",
    "lab_report",
    "reporting",
];
const WW_SUBCATEGORIES = [
    "assignment",
    "seatwork",
];
...
function countLabShare(termSlots) {
    if (termSlots.length === 0)
        return 0;
    const labLike = termSlots.filter((slot) => slot.sessionType === "laboratory" || slot.sessionType === "mixed").length;
    return labLike / termSlots.length;
}
function buildChapterEndingLessonOrders(tocUnits) {
    const endingOrders = new Set();
...
function pickBalancedPerformanceTaskSubtype(input) {
    ...
    const anchorIndex = termLessons.length === 0
        ? 0
        : Math.min(termLessons.length - 1, Math.max(0, Math.floor(((ptOrder - 0.5) / Math.max(1, counts.__targetTotal ?? 1)) * termLessons.length)));
    ...
    for (const subtype of PT_SUBCATEGORIES) {
        ...
        if (subtype === "lab_report")
            score += labShare * 8 + (anchorLesson?.preferredSessionType === "laboratory" ? 5 : 0);
        if (subtype === "reporting")
            score += (anchorDifficulty >= 3 ? 7 : 0) + (nearChapterEnd ? 4 : 0);
        if (subtype === "activity")
            score += anchorDifficulty <= 2 ? 3 : 0;
        if (score > bestScore) {
            best = subtype;
            bestScore = score;
        }
...
function pickBalancedWrittenWorkSubtype(input) {
    ...
    const anchorIndex = termLessons.length === 0
        ? 0
        : Math.min(termLessons.length - 1, Math.max(0, Math.floor(((wwOrder - 0.5) / Math.max(1, counts.__targetTotal ?? 1)) * termLessons.length)));
    const anchorLesson = termLessons[anchorIndex] ?? null;
    const anchorDifficulty = difficultyWeight(anchorLesson?.difficulty ?? "medium");
...
function toPacingPlan(input) {
    return (0, buildPacingPlan_1.buildPacingPlan)({
        slots: input.slots ?? [],
