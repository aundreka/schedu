...
function placementSignature(slot) {
    return slot.placements
        .map((placement) => `${placement.lane}:${placement.blockId}`)
        .sort()
        .join(",");
}
function stateProgressScore(state) {
    return (state.lessonOffset * 10000 +
        state.quizOffset * 3000 +
        state.ptOffset * 2000 +
        state.wwOffset * 1000 +
...
function addPlacement(slot, block, lane = "major") {
...
function getMajorBlockAt(slots, index, blocks) {
...
function getLastLessonIndex(slots, blocks) {
...
function normalizePlacementOrder(slot, blocks) {
    ...
    slot.placements = [...slot.placements].sort((a, b) => rank(a) - rank(b) || a.blockId.localeCompare(b.blockId));
    rebuildPlacementIds(slot);
}
function countPlacedMajorSlots(slots) {
    return slots.filter((slot) => slotHasMajorBlock(slot)).length;
}
function countPlacedMinorBlocks(slots) {
    return slots.reduce((sum, slot) => sum + getMinorPlacements(slot).length, 0);
}
function countFreeMajorSlots(slots) {
    return slots.filter((slot) => !slot.locked && !slotHasMajorBlock(slot)).length;
}
function freeMajorSlotIndexes(slots, startIndex, predicate) {
    const indexes = [];
    for (let index = startIndex; index < slots.length; index += 1) {
        const slot = slots[index];
        if (slot.locked || slotHasMajorBlock(slot) || !predicate(slot))
            continue;
        indexes.push(index);
    }
    return indexes;
}
function candidateLessonSlots(slots, startIndex, count, hasLabSlots) {
...
function chooseCombinations(indexes, count, startAt = 0, prefix = [], result = []) {
...
function hasWrittenWorkOnSlot(slot, blocks) {
    return getMinorPlacements(slot).some((placement) => {
        const block = findBlockById(blocks, placement.blockId);
        return block?.type === "written_work" && block.subcategory !== "quiz";
    });
}
function candidatePerformanceTaskSlots(slots, cycleLessonIndexes, hasLabSlots) {
    if (cycleLessonIndexes.length === 0)
        return [];
    ...
    const fallback = freeMajorSlotIndexes(slots, Math.max(0, cycleLessonIndexes[0] + 1), () => true);
    if (!hasLabSlots)
        return fallback;
    const lab = fallback.filter((index) => isLabFriendly(slots[index]));
    return lab.length > 0 ? lab : fallback;
}
function candidateWrittenWorkSlots(slots, cycleLessonIndexes, blocks) {
    return cycleLessonIndexes.filter((index) => !hasWrittenWorkOnSlot(slots[index], blocks));
}
function candidateQuizSlots(slots, afterIndex) {
    return freeMajorSlotIndexes(slots, Math.max(0, afterIndex + 1), () => true);
}
function candidateFinalQuizSlots(slots, afterIndex, reserveReviewSlot) {
    const examIndex = Math.max(0, slots.length - 1);
    const targetIndex = Math.max(Math.max(0, afterIndex + 1), examIndex - (reserveReviewSlot ? 2 : 1));
...
function moveMajorPlacement(slots, fromIndex, toIndex) {
    const source = slots[fromIndex];
    const destination = slots[toIndex];
    if (!source || !destination)
        return false;
    const major = getMajorPlacement(source);
    if (!major || destination.locked || slotHasMajorBlock(destination))
        return false;
...
function countLessonMajors(slots, fromIndex, toIndex, blocks) {
    let count = 0;
...
function getPreviousQuizIndex(slots, beforeIndex, blocks) {
...
function canMoveMajorLeft(slots, sourceIndex, targetIndex, blocks, hasLabSlots) {
    const sourceSlot = slots[sourceIndex];
    const targetSlot = slots[targetIndex];
    const major = getMajorPlacement(sourceSlot);
    if (!major || targetSlot.locked || slotHasMajorBlock(targetSlot))
        return false;
    ...
    if (hasLabSlots && block.type === "lesson") {
        if (isLectureFriendly(targetSlot))
            return true;
...
function compactTermSlotsLeft(termSlots, blocks, hasLabSlots) {
    for (let index = 0; index < termSlots.length; index += 1) {
        ...
        if (targetSlot.placements.length === 0) {
            for (let sourceIndex = index + 1; sourceIndex < termSlots.length; sourceIndex += 1) {
                ...
                if (sourceSlot.locked || sourceSlot.placements.length === 0)
                    continue;
                ...
                if (movableMinor) {
                    sourceSlot.placements = sourceSlot.placements.filter((placement) => placement.blockId !== movableMinor.blockId);
            ...
            for (let sourceIndex = index + 1; sourceIndex < termSlots.length; sourceIndex += 1) {
                const sourceSlot = termSlots[sourceIndex];
                if (sourceSlot.locked || !slotHasMajorBlock(sourceSlot))
                    continue;
                if (!canMoveMajorLeft(termSlots, sourceIndex, index, blocks, hasLabSlots))
                    continue;
                moveMajorPlacement(termSlots, sourceIndex, index);
...
function ensureLastQuizAfterLessons(termSlots, blocks) {
    ...
    while (lastQuizIndex >= 0 && lastLessonIndex > lastQuizIndex) {
        ...
        if (targetIndex === -1)
            break;
        if (!moveMajorPlacement(termSlots, lastQuizIndex, targetIndex))
            break;
...
function ensureFinalQuizBeforeExamWindow(termSlots, blocks) {
    ...
    if (finalQuizIndex < 0 || finalQuizIndex === targetIndex)
        return;
...
function movePlacementToSlot(sourceSlot, targetSlot, blockId, lane) {
...
function insertMajorAt(slots, targetIndex, block) {
    let emptyIndex = -1;
    ...
    if (!target || target.locked || slotHasMajorBlock(target))
        return false;
    addPlacement(target, block, "major");
    return true;
}
function finalizeSlots(slots, blocks, hasLabSlots) {
...
function buildExpansionCandidates(blocks) {
    const lessonExtensions = blocks
        .filter((block) => block.metadata.extraCandidateType === "lesson_extension")
        .sort((a, b) => Number(Boolean(b.metadata.highComplexity)) - Number(Boolean(a.metadata.highComplexity)) ||
    ...
    const ptExtensions = blocks
        ...
        .sort((a, b) => Number(Boolean(b.metadata.prioritizeReporting)) -
    ...
    const quizReviews = blocks
        ...
        .sort((a, b) => Number(b.metadata.unresolvedHardRegionCount ?? 0) -
...
function buildExpansionStates(state, candidate, termBlocks, coreSlots) {
    const nextStates = [];
    ...
    if (candidate.metadata.extraCandidateType === "review_before_quiz") {
        ...
        const quiz = termBlocks.find((block) => block.subcategory === "quiz" && Number(block.metadata.quizOrder ?? 0) === quizOrder);
        if (!quiz)
            return nextStates;
    ...
    if (candidate.metadata.extraCandidateType === "pt_extension") {
        ...
        const base = termBlocks.find((block) => block.type === "performance_task" &&
    ...
    if (candidate.metadata.extraCandidateType === "lesson_extension") {
        ...
        const base = termBlocks.find((block) => block.type === "lesson" &&
            !block.metadata.extraCandidateType &&
...
function buildCompressionStates(state, termBlocks) {
    const nextStates = [];
    ...
    for (const ww of writtenWorks) {
        ...
        for (const lesson of lessons) {
            if (Number(lesson.metadata.lessonOrder ?? 0) <= 1)
                continue;
            const lessonSlotIndex = findPlacementIndex(state.slots, lesson.id);
            if (lessonSlotIndex < 0 || lessonSlotIndex === wwSlotIndex)
                continue;
            const next = cloneState(state);
            if (movePlacementToSlot(next.slots[wwSlotIndex], next.slots[lessonSlotIndex], ww.id, "minor")) {
                next.balanceRemaining += 1;
                nextStates.push(next);
            }
        }
    ...
    if (performanceTasks.length > 2) {
        for (const pt of performanceTasks) {
            ...
            if (ptSlotIndex <= 0)
                continue;
            const next = cloneState(state);
            if (movePlacementToSlot(next.slots[ptSlotIndex], next.slots[ptSlotIndex - 1], pt.id, "minor")) {
                next.balanceRemaining += 1;
                nextStates.push(next);
            }
        }
...
function evaluateOutcome(state, lessons, performanceTasks, writtenWorks, quizzes, termBlocks, hasLabSlots) {
    ...
    const complete = state.lessonOffset >= lessons.length &&
    ...
    const score = (complete ? 1000000 : 0) +
        state.lessonOffset * 10000 +
        state.quizOffset * 3000 +
        state.ptOffset * 2000 +
        state.wwOffset * 1000 +
        Math.max(0, 500 - Math.abs(state.balanceRemaining) * 200) +
...
function pickBetterOutcome(current, next) {
    if (!current)
        return next;
...
function searchTermPlacements(input) {
    const { lessons, performanceTasks, writtenWorks, quizzes, termBlocks, hasLabSlots, lessonInterval, expansionCandidates, } = input;
    ...
    function recurse(state) {
        ...
        if (state.lessonOffset >= lessons.length &&
            state.ptOffset >= performanceTasks.length &&
            state.wwOffset >= writtenWorks.length &&
            state.quizOffset >= quizzes.length) {
        ...
        if (state.phase === "lesson") {
            ...
            for (const combo of lessonCombos) {
                ...
                if (seenChildStates.has(childSignature))
                    continue;
        ...
        if (state.phase === "pt") {
            ...
            const ptCandidates = candidatePerformanceTaskSlots(state.slots, state.cycleLessonIndexes, hasLabSlots);
            const seenChildStates = new Set();
        ...
        if (state.phase === "expand") {
            ...
            best = pickBetterOutcome(best, recurse({ ...state, extraIndex: state.extraIndex + 1 }));
...
function placeBlocks(input) {
    ...
    for (const [, termSlotsRaw] of getTermSlots(slots)) {
        ...
        const writtenWorks = sortByMetadataOrder(findBlock(input.blocks, "written_work", termIndex, (block) => block.subcategory !== "quiz"), "wwOrder");
        const quizzes = sortByMetadataOrder(findBlock(input.blocks, "written_work", termIndex, (block) => block.subcategory === "quiz"), "quizOrder");
        const performanceTasks = sortByMetadataOrder(findBlock(input.blocks, "performance_task", termIndex), "ptOrder");
        ...
        const lessonInterval = Math.max(1, Number(exam?.metadata.lessonInterval ?? quizzes[0]?.metadata.lessonInterval ?? 1));
        const examSlot = exam && exam.subcategory === "final" && typeof exam.metadata.preferredDate === "string"
